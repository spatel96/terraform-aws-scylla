...
while sleep 5; do
	if cqlsh -u cassandra -p cassandra -e "CREATE USER IF NOT EXISTS ${admin} WITH PASSWORD '${admin_password}' SUPERUSER;" &&
		...
		cqlsh -u ${admin} -p "${admin_password}" -e "DROP USER cassandra;"; then
		break
	fi
done
